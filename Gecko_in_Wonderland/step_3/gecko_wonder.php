...
foreach ($intermediate as $step) {
    $tree = [];
    $tree[0][0] = 0;
    reset_grid();
    $finish = find_finish($step);
...
function find_finish ($step) {
    global $size;
    global $origin_grid;
    for ($count_row = 0; $count_row < $size[1]; $count_row++) {
        $count_column = 0;
        while ($count_column < $size[0]) {
            if ($origin_grid[$count_row][$count_column] == $step) {
                return array($count_row, $count_column);
            }
            $count_column++;
        }
    }
...
function build_grid() {
    ...
    for ($count_row = 0; $count_row < $size[1]; $count_row++) {
        ...
        while ($count_column < $size[0]) {
            ...
            } elseif ($grid[$count_row][$count_column] != "#" && $grid[$count_row][$count_column] != " ") {
                array_push($intermediate, $grid[$count_row][$count_column]);
            }
            $count_column++;
        }
    }
...
function reset_grid() {
    ...
    for ($count_row = 0; $count_row < $size[1]; $count_row++) {
        $count_column = 0;
        while ($count_column < $size[0]) {
            if ($origin_grid[$count_row][$count_column] == ".") {
                $origin_grid[$count_row][$count_column] = " ";
            }
